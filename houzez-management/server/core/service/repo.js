...
export class CoreRepo {
  ...
  getBaseConditionByRole() {
    ...
    if (!user) return cond;
    if (!this._isApplyFilter()) {
      return cond;
    }

    if (
      this.filterByStore &&
      Array.isArray(user.store_uuids) &&
      user.store_uuids.length > 0
    ) {
      cond[this.storeFieldName] = {
        $in: this.context.currentUser.store_uuids,
      };

      return cond;
    }

    if (typeof user.client_uuid === 'string' && user.client_uuid.length > 0) {
      cond.client_uuid = user.client_uuid;
