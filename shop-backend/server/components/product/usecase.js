...
export class ProductUsecase extends CoreUsecase {
  ...
  async list({
    ...
    try {
      ...
      if (category_id && category_id > 0) {
        temp = this._buildQueryWihCategory({ category_id, tag_ids, keyword });
      } else {
        temp = this._buildQuery({ category_id, tag_ids, keyword });
      }
      let { queryText, params } = temp;
      params = [...params, sort_by, sort_dir, LIMIT, SKIP];
      const result = await this.repo.executeQuery(queryText, params);
      const doc = Common.populateResult(result, 'total_count');
      return {
        data: doc.data,
        total: doc.total,
        page: PAGE,
  ...
  async getDetail(product_id) {
    try {
      ...
      const temp = await this.repo.executeQuery(queryText, params);
  ...
  _buildQueryWiclientConnecthCategory({ category_id, tag_ids, keyword }) {
    let condition = '';
    let params = [];
